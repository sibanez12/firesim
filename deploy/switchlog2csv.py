...
def parse_switchlog(switchlog):
    ...
    resp_time_stats = {"service_time":[], "resp_time":[], "sent_time":[], "recv_time":[], "time":[], "src_context":[], "load":[]}
    for line in readlines:
        if "&&CSV&&QueueSize" in line:
            ...
            resp_time_stats["src_context"].append(float(data[6]))
            resp_time_stats["load"].append(float(data[7]))
    return qsize_stats, event_stats, resp_time_stats
...
def main():
    ...
    qsize_stats, event_stats, resp_time_stats = parse_switchlog(args.switchlog)
